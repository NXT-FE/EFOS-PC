...
<br/>
增加了 **react-router** /**antd**，后期根据需求可引入react-redux(全局数据store)
...
## 界面配置化
同一类组件可能拥有多种样式/事件/接口参数/接口/，模版样式也是如此，如果一律通过配置生成界面，需要配置的数据太繁杂、耦合度太高，同级组件之间传递数据困难，维护困难，配置也麻烦
- 初步结论
